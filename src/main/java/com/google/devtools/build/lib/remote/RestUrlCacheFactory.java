...
/**
 * A factory class for providing a {@link ConcurrentMap} object implemented by a REST service.
 * The URL has to support PUT, GET, and HEAD operations
 */
  ...
  private static class RestUrlCache implements ConcurrentMap<String, byte[]> {
    ...
    public byte[] get(Object key) {
      try {
        ...
        int statusCode = response.getStatusLine().getStatusCode();
        if (HttpStatus.SC_NOT_FOUND == statusCode) return null;
    ...
    public byte[] put(String key, byte[] value) {
      try {
        ...
        if (HttpStatus.SC_OK != statusCode) throw new RuntimeException("PUT failed with status code " + statusCode);
      } catch (IOException e) {
        throw new RuntimeException(e);
      }
      return null;
